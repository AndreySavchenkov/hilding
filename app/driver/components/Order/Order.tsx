...
export const Order = ({ item, setOptions }: Props) => {
  const orderSource = formatOrderSource(item.lineOptions, item.machineOptions);
  const areaDisplay = item.areaOptions
    ? `(${AreaOptionsEnum[item.areaOptions]})`
    : "";
